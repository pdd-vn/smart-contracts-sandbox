...
contract LendingMachine is LendingMachineBase {
    ...
    function mint_new_nft(string memory uri) public returns(uint256){
        NFT memory _nft;
    ...
    function deposit(uint256 token_id, uint256 price) public {
        require(nft_id_mapping[token_id].owner == msg.sender, "ERROR: Insufficient ownership");
    ...
    function lend(uint256 token_id) public {
        ...
        require(
            IERC20(ERC20_ADDRESS).allowance(msg.sender, address(this)) >= nft_id_mapping[token_id].price,
            "ERROR: Insufficient allowance"
        );

        require(
            IERC20(ERC20_ADDRESS).balanceOf(msg.sender) >= nft_id_mapping[token_id].price,
